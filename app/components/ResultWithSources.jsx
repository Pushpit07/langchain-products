	...
	return (
		<div className={`flex flex-col ${isLast ? "flex-grow" : ""}`}>
			<div className="flex mb-4">
				...
				<p className={message.type === "user" ? "user" : "bot"} style={{ maxWidth: "90%" }}>
					{message.text}
				</p>
			</div>
			...
			{message.sourceDocuments && (
				<div className="mb-6">
					<button className="text-gray-600 text-sm font-bold" onClick={() => setShowSources(!showSources)}>
						Source Documents {showSources ? "(Hide)" : "(Show)"}
					</button>
					{showSources &&
						message.sourceDocuments.map((document, docIndex) => (
							<div key={docIndex}>
								<h3 className="text-gray-600 text-sm font-bold">Source {docIndex + 1}:</h3>
								<p className="text-gray-800 text-sm mt-2">{document.pageContent}</p>
								<pre className="text-xs text-gray-500 mt-2">{JSON.stringify(document.metadata, null, 2)}</pre>
							</div>
						))}
				</div>
			)}
...
const ResultWithSources = ({ messages, pngFile, maxMsgs }) => {
	...
	return (
		...
		>
			{messages && messages.map((message, index) => <MessageItem key={index} message={message} pngFile={pngFile} />)}
		</div>
	);
};
